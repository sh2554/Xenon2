...
export default function XenonIDE() {
  ...
  return (
    <div className="space-y-6">
      {userIsPro && (
        <div className="border border-[var(--border-strong)] bg-[var(--panel-muted)] p-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <PlanBadge plan={profile?.plan} size="md" />
            <p className="text-xs font-bold uppercase tracking-wider text-[var(--text)]">
              {userIsMax
                ? "Max plan active — full teacher & lab suite."
                : "Pro plan active — unlimited projects, full past papers, extended theory & IDE skins."}
            </p>
          </div>
        </div>
      )}
      <motion.section className="xenon-panel p-6 border border-[var(--border)] bg-[var(--panel)]" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--border)] pb-6 mb-6">
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tight flex items-center gap-3 flex-wrap">
              PYTHON IDE & LABS
              {userIsPro && <PlanBadge plan={profile?.plan} size="sm" />}
            </h2>
            <p className="mt-1 text-xs text-[var(--muted)]">Write Python code on the left and inspect execution output on the right.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <button className="xenon-btn text-xs tracking-widest px-6" onClick={runCode} disabled={isRunning}>
              {isRunning ? <History className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5 fill-current" />}
              {isRunning ? "RUNNING..." : "RUN CODE"}
            </button>
            <button className="xenon-btn-ghost text-xs tracking-widest" onClick={() => setShowVariables((v) => !v)}>
              <Database className="h-3.5 w-3.5" />
              VARIABLES
            </button>
            <button className="xenon-btn-ghost text-xs tracking-widest" onClick={onSave}>
              <Save className="h-3.5 w-3.5" />
              SAVE
            </button>
            <button
              className="xenon-btn-ghost text-xs tracking-widest"
              onClick={async () => {
            ...
            <button className="xenon-btn-subtle text-xs tracking-widest" onClick={openProSkins}>
              <Layers className="h-3.5 w-3.5" /> PRO SKINS {!canUseProSkins && "(PRO)"}
            </button>
            <button className="xenon-btn-subtle text-xs tracking-widest" onClick={() => setShowChallenge((v) => !v)}>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input
            className="xenon-input max-w-sm text-xs font-mono font-bold"
            value={activeProjectTitle}
            onChange={(e) => setActiveProjectTitle(e.target.value)}
            placeholder="main.py"
          />
          <span className="xenon-badge">Standard Python modules available</span>
          {saveStatus && <span className="text-xs font-mono text-[var(--muted)]">{saveStatus}</span>}
          {shareStatus && (
            <span
              className={`text-xs font-mono flex items-center gap-1 ${shareStatus.startsWith("Share link:") ? "cursor-pointer select-all" : ""} text-[var(--accent)]`}
            ...
            <div className="p-6 border-b border-[var(--border)] flex items-center justify-between bg-gradient-to-r from-[var(--accent-soft)] to-transparent">
              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-2">
                  {[
                    { id: "default", label: "Default Theme", bg: "linear-gradient(135deg, #0E0E0E, #161616)" },
