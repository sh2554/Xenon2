        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-2xl">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[11px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-white/5" style={{ color: unit.accent }}>
                {unit.unit}
              </span>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[var(--panel-soft)] text-[var(--muted)] border border-[var(--border)] font-mono">
                {unit.paperCode} • {unit.paperTitle}
              </span>
            </div>
            ...
            {unit.subtopics && unit.subtopics.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {unit.subtopics.map((sub, sIdx) => (
                  <span key={sIdx} className="text-[11px] px-2.5 py-0.5 rounded-lg bg-[var(--panel-soft)] text-[var(--muted)] border border-[var(--border)] font-medium">
                    {sub}
                  </span>
                ))}
              </div>
            ...
            {totalSections > 0 && (
              <div className="flex items-center gap-2 mt-3">
                ...
                <span className="text-[10px] font-bold text-[var(--muted)]">{completedCount}/{totalSections} Read</span>
              </div>
            )}
          </div>
    ...
    >
      <div className="rounded-[22px] bg-[var(--panel)] p-6 h-full flex flex-col">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-black uppercase tracking-[0.15em] px-3 py-1 rounded-full bg-white/5" style={{ color: unit.accent }}>
              {unit.unit}
            </span>
            <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-[var(--panel-soft)] text-[var(--muted)] border border-[var(--border)]">
              {unit.paperCode || (unit.paper === 1 ? "J277/01" : "J277/02")}
            </span>
          </div>
        ...
        <h3 className="text-xl font-black tracking-tight leading-tight mb-2 group-hover:text-[var(--accent)] transition-colors">{unit.title}</h3>
        
        {unit.subtopics && unit.subtopics.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            {unit.subtopics.slice(0, 3).map((sub, sIdx) => (
              <span key={sIdx} className="text-[10px] px-2 py-0.5 rounded-md bg-[var(--panel-soft)] text-[var(--muted)] font-medium">
                {sub.replace(/^\d+\.\d+\.\d+\s*/, "")}
              </span>
            ))}
            {unit.subtopics.length > 3 && (
              <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-[var(--panel-soft)] text-[var(--muted)]">
...
export default function TheoryPanel({ initialPaper = null, initialUnitId = null, onClearTarget = null }) {
  const [selectedId, setSelectedId] = useState(initialUnitId);
  const [selectedPaper, setSelectedPaper] = useState(initialPaper || "all"); // 'all' | 1 | 2
  const [savedProgress, setSavedProgress] = useState(() => loadProgress());

  useEffect(() => {
    if (initialUnitId !== undefined && initialUnitId !== null) {
      setSelectedId(initialUnitId);
    }
  }, [initialUnitId]);

  useEffect(() => {
    if (initialPaper !== undefined && initialPaper !== null) {
  ...
  const paper1Units = useMemo(() => THEORY_UNITS.filter((u) => u.paper === 1), []);
  const paper2Units = useMemo(() => THEORY_UNITS.filter((u) => u.paper === 2), []);
  ...
  if (selected) {
    return <TopicDetail unit={selected} onBack={handleBack} />;
  }
  ...
  return (
    <motion.div className="space-y-8 pb-20" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3">
              <span className="xenon-pill bg-[var(--accent-soft)] text-[var(--accent)] border-none px-4 py-1.5 flex items-center gap-2">
                ...
                OCR GCSE (J277) Curriculum
              </span>
              <span className="xenon-pill bg-[var(--panel-soft)] text-[var(--muted)] border border-[var(--border)] px-3 py-1.5">
                2 Exam Papers (50% Each)
              </span>
            </div>
            <h2 className="mt-6 text-4xl font-black tracking-tight sm:text-5xl leading-[1.1]">The Theory Hub</h2>
            <p className="mt-4 text-base sm:text-lg text-[var(--muted)] font-medium leading-relaxed">
              Master both OCR GCSE Computer Science exam papers with precision notes and interactive flashcards. Structured around Paper 1 (Computer Systems) and Paper 2 (Computational Thinking &amp; Algorithms).
            </p>
            
            <div className="mt-8 flex flex-wrap gap-8">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-[var(--accent-soft)] flex items-center justify-center text-[var(--accent)] border border-[var(--accent)]/20">
                  <BookMarked className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xl font-black">{THEORY_UNITS.length} Units</p>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">Across 2 Papers</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">Flashcards</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-[var(--success)]/10 flex items-center justify-center text-[var(--success)] border border-[var(--success)]/20">
                  <Check className="h-5 w-5" />
                </div>
                <div>
                  <p className="text-xl font-black">100%</p>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">OCR J277 Spec</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {continueUnit && (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          className="xenon-panel p-5 relative overflow-hidden cursor-pointer group rounded-2xl"
        ...
        >
          ...
          <div className="relative flex items-center gap-4 flex-wrap">
            <div className="h-12 w-12 rounded-xl flex items-center justify-center" style={{ background: `${continueUnit.accent}20` }}>
              <Play className="h-5 w-5" style={{ color: continueUnit.accent }} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--muted)]">Continue Studying</span>
                <span className="text-[9px] font-bold px-2 py-0.2 rounded bg-white/10 text-[var(--accent)] font-mono">{continueUnit.paperCode}</span>
              </div>
      ...
      {/* ─── OCR GCSE Specification Paper Tabs ─── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <span>All Units ({THEORY_UNITS.length})</span>
          </button>
          <button
          ...
          >
            ...
            <span>Paper 1: Computer Systems (J277/01)</span>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-black/20 font-bold">{paper1Units.length} Units</span>
          </button>
          <button
          ...
          >
            ...
            <span>Paper 2: Algorithms &amp; Programming (J277/02)</span>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-black/20 font-bold">{paper2Units.length} Units</span>
          </button>
        </div>

        <span className="text-xs text-[var(--muted)] font-medium">
          OCR GCSE J277 • 2 Exam Papers
        </span>
      </div>

      {/* ─── Paper 1 Section ─── */}
      {(selectedPaper === "all" || selectedPaper === 1) && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between pb-3 border-b border-[var(--border)] gap-1">
            <div className="flex items-center gap-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.6)] shrink-0" />
              <h3 className="text-lg font-black tracking-tight text-[var(--text)]">
                Paper 1: Computer Systems <span className="text-xs font-semibold text-[var(--muted)] font-mono ml-2">(OCR J277/01 • 50% of GCSE)</span>
              </h3>
            </div>
            <span className="text-xs text-[var(--muted)]">Hardware, Architecture, Storage, Networks, Security &amp; Ethics</span>
          </div>

          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {paper1Units.map((unit) => renderUnitCard(unit))}
          </div>
        </div>
      )}

      {/* ─── Paper 2 Section ─── */}
      {(selectedPaper === "all" || selectedPaper === 2) && (
        <div className="space-y-4 pt-4">
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between pb-3 border-b border-[var(--border)] gap-1">
            <div className="flex items-center gap-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-fuchsia-400 shadow-[0_0_8px_rgba(232,121,249,0.6)] shrink-0" />
              <h3 className="text-lg font-black tracking-tight text-[var(--text)]">
                Paper 2: Computational Thinking, Algorithms &amp; Programming <span className="text-xs font-semibold text-[var(--muted)] font-mono ml-2">(OCR J277/02 • 50% of GCSE)</span>
              </h3>
            </div>
            <span className="text-xs text-[var(--muted)]">Algorithms, Searching &amp; Sorting, Python Coding, Logic Gates &amp; IDE Tools</span>
          </div>

          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {paper2Units.map((unit) => renderUnitCard(unit))}
          </div>
        </div>
      )}
    </motion.div>
