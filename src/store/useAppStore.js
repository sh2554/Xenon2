...
export const useAppStore = create((set, get) => ({
  ...
  hydrateUserSession: async (sessionUser) => {
    ...
    // Mark hydration complete so the dashboard can render
  ...
  startUserHydration: (sessionUser) => {
    ...
    // Mark hydration in-flight so dashboard doesn't render prematurely
    set({ user: sessionUser, authHydrated: false, completedPracticeSkills: {} });
    get().hydrateUserSession(sessionUser)
      .then(() => {
        set({ authHydrated: true });
      })
      .catch((err) => {
  ...
  ensureProfileExists: async (sessionUser) => {
    ...
    try {
      ...
      const usernameClean = (md.username || "").replace(/[^a-zA-Z0-9_]/g, "").slice(0, 20);
      const username = usernameClean || `${fallbackName.replace(/[^a-zA-Z0-9_]/g, "").slice(0, 14) || "xenonuser"}${Math.floor(Math.random() * 900 + 100)}`;
      ...
      if (insertErr) {
        console.warn("ensureProfileExists upsert warning:", insertErr);
        if (insertErr.message?.includes("username") || insertErr.code === "23505") {
  ...
  loadProfile: async (sessionUser) => {
    ...
    const needsSetup = get().shouldPromptProfileSetup(normalized, user);
    set({
      profile: normalized,
      showProfileSetup: needsSetup,
      showInitOverlay: !needsSetup && !normalized.has_seen_init,
    });
    return normalized;
  },
  ...
  finishInitOverlay: async () => {
    const { profile } = get();
    set({ showInitOverlay: false });
    if (!profile) return;
    try {
      await supabase.from("profiles").update({ has_seen_init: true }).eq("id", profile.id);
      set((state) => ({
        profile: state.profile ? { ...state.profile, has_seen_init: true } : state.profile,
      }));
    } catch (e) {
      console.warn("Failed to persist has_seen_init:", e);
    }
  },
  ...
  deleteProject: async (projectId) => {
    ...
    set((state) => ({
      ...
      activeProjectId: activeProjectId === projectId ? null : state.activeProjectId,
      activeProject: activeProjectId === projectId ? { title: "Untitled.py", code: "" } : state.activeProject,
    }));
  },
  ...
  renameProject: async (projectId, newTitle) => {
    ...
    if (!user || !projectId || !cleanTitle) return;
    const { error } = await supabase
      .from("projects")
    ...
    if (error) throw error;
    set((state) => ({
      projects: state.projects.map((p) => (p.id === projectId ? { ...p, title: cleanTitle } : p)),
      activeProject:
        state.activeProjectId === projectId
          ? { ...state.activeProject, title: cleanTitle }
          : state.activeProject,
    }));
  },

  duplicateProject: async (project) => {
    const { user, profile, projects } = get();
    if (!user || !project) return;
    const limit = getProjectLimit(profile?.plan);
    if (limit != null && (projects?.length ?? 0) >= limit) {
      throw new Error(`Free plan allows up to ${limit} saved projects. Upgrade to Pro or Max in Account Settings.`);
    }
    const cleanTitle = `${project.title || "Untitled"}`.replace(/(\.py)?$/, " (Copy)$1");
    const payload = {
      owner_id: user.id,
      title: cleanTitle,
      code: project.code || "",
      snippet: (project.code || "").slice(0, 120),
      updated_at: new Date().toISOString(),
    };
